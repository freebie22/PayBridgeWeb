...
import { personalAccountHolderProfileModel, userToCompanyTransactionModel } from '../../Interfaces';
import { useSelector } from 'react-redux';
import { RootState } from '../../Storage/Redux/store';

interface userToCompanyProps{
    transaction: userToCompanyTransactionModel
}

function UserToCompanyTransactionCard(props: userToCompanyProps) {
    ...
    return (
      <div className="card mb-4" style={{border: "1px solid #0DA378", backgroundColor: "#212529"}}>
        ...
        <div className="card-body" style={{color: "#0DA378"}}>
          ...
          <p className="card-text">{props.transaction.senderHolderId === profileData.accountId ? "Номер Вашої картки:" : "Номер картки відправника:"}  <span className='text-white'>{props.transaction.senderBankCardNumber}</span> </p>
          <p className="card-text">{props.transaction.receiverHolderId === profileData.accountId ? (<div>Переказ здійснено на рахунок компанії:  <span className='text-white'>{props.transaction.receiverCompanyShortName}</span> </div>) : (<div>Переказ здійснено з рахунку клієнта: <span className='text-white'>{props.transaction.senderCredentials}</span></div>)}</p>
        ...
        <button className='btn btn-warning mx-auto mb-3' onClick={() => navigate(`/myProfile/transactionDetails/${props.transaction.transactionUniqueNumber}/userToCompanyTransaction`)}>Детальніше</button>
      </div>
    )
}

export default UserToCompanyTransactionCard